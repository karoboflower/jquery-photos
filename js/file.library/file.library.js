; (function ($, window, document, undefined) {
     ...
     */
    /**
     * 自己封装的其他方法，这个地方用不到，可能上传图片需要用到token或者其他唯一标识的地方
    ...
    */
    ...
    FileLibrary.prototype = {
        ...
        uploadImagesEvent: function () {
            ...
            //我自己用的是插件，这个可以自己写，上传成功后，写上如下代码就行
            var _this = this;
            _this.$element.find('.j-upload').on('click',function(){
                var $list = _this.$element.find('ul.file-list-item');
                var data=[{"fileId":273,
                "groupId":70,
                "fileBucket":"tianlun-wp",
                "fileName":"0e44ea9151a0444393efe7422d8a5aa7.jpg",
                "fileSize":210346,"fileSuffix":"jpg",
                "fileUrl":"http://qiniu.mtlyy.com/0e44ea9151a0444393efe7422d8a5aa7.jpg-cmp",
                "extension":"",
                "shopId":1005,
                "delFlag":"0",
                "createTime":"2020-01-02 21:28:41",
                "updateTime":"2020-01-02 21:28:41"}
            ];
                $list.prepend(template('tpl-file-list-item', { data: data }));
                layer.msg('上传成功');
            })
            // var _this = this;
            // var loadIndex = null;
        ...
        getJsonData: function (params, callback) {
            ...
            $.ajax({
                url: '/jquery-photos/ajax/group.json',
                type: 'get',
                success: function (res) {
                    layer.close(loadIndex);
                    if (res.code === 0) {
        ...
        */
        getImageData: function (params) {
            return new Promise(function (resolve, reject) {
                var loadIndex = layer.load();
                $.ajax({
                    url: '/jquery-photos/ajax/photo.json',
                    type: 'get',
                    data: params,
                    success: function (res) {
                        layer.close(loadIndex);
